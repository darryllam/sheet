...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    local/data_prep.py \
        --original-path "${db_root}/metadata.csv" \
        --wavdir "${db_root}" --out "data/urgent2024_mos_train.csv" \
        --setname "train" --seed "${seed}" \
        --resample --target-sampling-rate "${target_sampling_rate}" --target-wavdir "${db_root}/wavs_${target_sampling_rate}" 
    local/data_prep.py \
        --original-path "${db_root}/metadata.csv" \
        --wavdir "${db_root}" --out "data/urgent2024_mos_dev.csv" \
...
if [ "${stage}" -le 2 ] && [ "${stop_stage}" -ge 2 ]; then
    ...
    ${cuda_cmd} --gpu "${n_gpus}" "${expdir}/train.log" \
        ${train} \
            --config "${conf}" \
            --train-csv-path "data/urgent2024_mos_train.csv" \
            --dev-csv-path "data/urgent2024_mos_dev.csv" \
            --outdir "${expdir}" \
            --resume "${resume}" \
            --verbose "${verbose}" \
            --seed "${seed}"
